...
Proses pengamanan data pada OpenVPN melibatkan mekanisme enkapsulasi berlapis yang secara signifikan mengubah anatomi struktural paket sebelum ditransmisikan melintasi jaringan internet fisik publik \citep{crist2015mastering}. Pada lalu lintas jaringan non-VPN standar, perangkat pengamat jaringan luar dapat dengan mudah memeriksa \textit{header} paket asli untuk membaca alamat IP tujuan akhir, tipe protokol, serta informasi \textit{port} aplikasi yang digunakan (seperti TCP port 443 untuk komunikasi HTTPS). 

Namun, ketika mekanisme enkapsulasi terowongan OpenVPN diterapkan, paket IP asli milik aplikasi—yang mencakup \textit{inner header} (alamat IP privat asal dan tujuan, serta port aplikasi internal asli) beserta data aplikasinya (\textit{payload})—akan dienkripsi secara penuh dan dibungkus di dalam muatan data aman VPN \citep{crist2015mastering}. Akibatnya, elemen metadata publik yang tersisa pada \textit{outer header} (pembungkus luar paket) dan dapat diamati secara terbuka oleh perangkat perantara (\textit{intermediate routers}) hanyalah alamat IP fisik klien terluar, alamat IP publik dari Server VPN, serta nomor port transportasi luar yang seragam \citep{crist2015mastering}. Umumnya, pembungkus luar ini menggunakan UDP port 1194 sebagai standar default OpenVPN atau TCP port 443 untuk menyamar sebagai lalu lintas web TLS standar \citep{crist2015mastering}. Penyembunyian identitas port aplikasi inilah yang melandasi urgensi penggunaan algoritma pembelajaran mesin untuk mengenali karakteristik perilaku statistik aliran data yang tersisa.
%-----------------------------------------------------------------------------%
