...
%-----------------------------------------------------------------------------%
Bagian ini menjelaskan konsep, teori, dan teknologi fundamental yang menjadi dasar metodologi \textit{Hybrid Flow Vector} (HFV).
...
\begin{enumerate}
  ...
  \item Pola statistik \textit{flow}: Karakteristik statistik yang dihitung dari agregasi paket dalam satu \textit{flow}, seperti durasi total, volume total data, serta distribusi statistik ukuran paket dan Waktu antar-kedatangan (inter-arrival times, IAT) \citep{azab2024network, dainotti2012issues}.
...
%-----------------------------------------------------------------------------%
%-----------------------------------------------------------------------------%
\section{Virtual Private Network (VPN) dan Protokol OpenVPN}
\label{sec:bab2-vpn-openvpn}
%-----------------------------------------------------------------------------%

Virtual Private Network (VPN) dirancang untuk memenuhi kebutuhan mendasar akan komunikasi data yang aman, privat, dan terautentikasi melintasi infrastruktur jaringan luas yang pada dasarnya bersifat tidak terlindungi \citep{crist2015mastering}. VPN bekerja dengan cara memperluas jaringan privat melintasi jaringan publik, memungkinkan pengguna mengirim dan menerima data seolah-olah perangkat mereka terhubung langsung ke jaringan privat tersebut melalui pembentukan jalur virtual terenkripsi atau \textit{tunneling} \citep{DraperGil2016}. Selain digunakan untuk mengamankan data korporasi, teknologi ini telah menjadi solusi populer bagi pengguna individu untuk menjaga privasi online, menghindari pengawasan pihak ketiga, serta menembus pembatasan geografis atau sensor konten \citep{DraperGil2016}.

\subsection{Mekanisme Kerja dan Saluran Protokol OpenVPN}
OpenVPN adalah salah satu protokol dan perangkat lunak VPN bersumber terbuka (\textit{open-source}) yang paling andal dan banyak diimplementasikan dalam arsitektur jaringan modern \citep{crist2015mastering}. Untuk menyediakan mekanisme keamanan tingkat tinggi, OpenVPN sangat bergantung pada pustaka kriptografi pihak ketiga seperti OpenSSL atau PolarSSL dalam mengeksekusi operasi enkripsi dan penandatanganan paket data \citep{crist2015mastering}. Secara operasional, arsitektur keamanan OpenVPN terbagi menjadi dua saluran utama yang bekerja secara komplementer untuk melindungi data:

\begin{itemize}
    \item \textit{Control Channel} (Saluran Kendali): Saluran ini berfungsi untuk melakukan proses negosiasi parameter keamanan, pertukaran kunci enkripsi (\textit{key exchange}), serta autentikasi awal antara klien dan server dengan memanfaatkan kerangka kerja \textit{Secure Socket Layer} (SSL) atau \textit{Transport Layer Security} (TLS) \citep{crist2015mastering, stallings2006cryptography}.
    \item \textit{Data Channel} (Saluran Data): Setelah jalur TLS berhasil dibangun secara aman pada \textit{control channel}, seluruh lalu lintas data aplikasi aktual pengguna akan dilewatkan melalui \textit{data channel} \citep{crist2015mastering}. Saluran data ini menjamin kerahasiaan (\textit{confidentiality}) melalui algoritma enkripsi simetris dan memastikan integritas pesan (\textit{message integrity}) menggunakan kode autentikasi pesan (\textit{Message Authentication Code} / MAC) \citep{stallings2006cryptography}.
\end{itemize}

\subsection{Lapisan Antarmuka Virtual (TUN dan TAP)}
\label{subsec:bab2-tun-tap}

Keunggulan fleksibilitas OpenVPN terletak pada kemampuannya beroperasi menggunakan penggerak virtual adapter (\textit{virtual network interface device drivers}) tingkat kernel, yaitu antarmuka TUN (\textit{network tunnel}) dan TAP (\textit{network tap}) \citep{crist2015mastering}. Kedua antarmuka virtual ini memiliki perbedaan fundamental pada lapisan operasional jaringan:

\begin{itemize}
    \item Antarmuka TAP: Beroperasi pada Lapisan Data Link (Layer 2 OSI) yang melewatkan paket bingkai Ethernet mentah (\textit{raw Ethernet frames}) dan mendukung penuh enkapsulasi jembatan jaringan (\textit{network bridging}) \citep{crist2015mastering}.
    \item Antarmuka TUN: Beroperasi pada Lapisan Network (Layer 3 OSI) yang bekerja secara spesifik untuk merutekan paket-paket IP standar jaringan \citep{crist2015mastering}.
\end{itemize}

Dalam implementasi model arsitektur klien/server standar, OpenVPN mengonfigurasi perangkat TUN untuk menerima peruteran seluruh lalu lintas data IP dari host klien ke dalam terowongan virtual privat \citep{crist2015mastering}. Server kemudian mengalokasikan alamat IP privat dinamis dari pangkalan data subnet kepada klien yang berhasil terautentikasi \citep{crist2015mastering}. Dataset ISCX 2016 yang menjadi subjek analisis utama dalam eksperimen penelitian ini dikumpulkan seluruhnya menggunakan mode antarmuka virtual TUN \citep{DraperGil2016}. 

Konsekuensi teknis dari metode pengumpulan data pada antarmuka virtual \texttt{tun0} ini sangat krusial; berkas lalu lintas (\textit{pcap}) yang terekam merepresentasikan paket data pada titik sebelum mengalami enkapsulasi luar oleh daemon OpenVPN (untuk arus keluar) atau tepat setelah de-kapsulasi dilakukan (untuk arus masuk). Batasan penangkapan (\textit{capture boundary}) ini menjadi alasan mengapa struktur \textit{inner header} jaringan tetap terbaca secara transparan pada dataset.

\subsection{Struktur Enkapsulasi dan Metadata OpenVPN}
\label{subsec:bab2-enkapsulasi}

Proses pengamanan data pada OpenVPN melibatkan mekanisme enkapsulasi berlapis yang secara signifikan mengubah anatomi struktural paket ketika ditransmisikan melintasi jaringan internet fisik publik \citep{crist2015mastering}. Pada lalu lintas jaringan non-VPN standar, perangkat pengamat jaringan luar dapat dengan mudah memeriksa \textit{header} paket asli untuk membaca alamat IP tujuan akhir, tipe protokol, serta informasi \textit{port} aplikasi yang digunakan (seperti TCP port 443 untuk komunikasi HTTPS). 

Ketika mekanisme enkapsulasi terowongan OpenVPN diterapkan pada jaringan fisik publik, paket IP asli milik aplikasi—yang mencakup \textit{inner header} (alamat IP privat asal dan tujuan, serta port aplikasi internal asli) beserta data aplikasinya (\textit{payload})—akan dienkripsi secara penuh oleh saluran data (\textit{data channel}) berbasis TLS dan dibungkus di dalam muatan data aman VPN \citep{crist2015mastering, stallings2006cryptography}. Akibatnya, elemen metadata publik yang tersisa pada \textit{outer header} (pembungkus luar paket) di jaringan publik hanyalah alamat IP fisik klien terluar, alamat IP publik dari Server VPN, serta nomor port transportasi luar yang seragam, seperti UDP port 1194 atau TCP port 443 \citep{crist2015mastering}.

\begin{figure}[h!]
    \centering
    \resizebox{\textwidth}{!}{% Mengubah ukuran agar pas dengan lebar margin halaman
    \begin{tikzpicture}[
        block/.style={draw, rectangle, minimum height=1.2cm, align=center, font=\small},
        outer/.style={fill=gray!20},
        ip/.style={fill=blue!20},
        tcp/.style={fill=green!20},
        app/.style={fill=yellow!30},
        enc/.style={fill=red!20},
        >=Stealth
    ]

    % --- BARIS 1: JARINGAN PUBLIK (eth0) ---
    \node[block, outer, minimum width=2.5cm] (out_ip) {Outer IP Header\\(IP Publik)};
    \node[block, outer, minimum width=2.5cm, right=0 of out_ip] (out_udp) {UDP Header\\(Port 1194)};
    \node[block, outer, minimum width=2.5cm, right=0 of out_udp] (ovpn) {OpenVPN\\Header / MAC};
    \node[block, enc, minimum width=6cm, right=0 of ovpn] (ovpn_payload) {Muatan Terenkripsi OpenVPN\\(\textit{Encrypted Payload})};

    % Judul Baris 1 (Diposisikan di tengah keseluruhan baris)
    \path (out_ip.north west) -- (ovpn_payload.north east) node[midway, above=0.3cm, font=\normalsize\bfseries] {Lalu Lintas di Jaringan Publik Luar (Antarmuka Fisik / \texttt{eth0})};

    % --- BARIS 2: DALAM TEROWONGAN (tun0) ---
    % Diturunkan jaraknya (below=2.8cm dari south west) agar tidak bertabrakan dengan garis/teks dekapsulasi
    \node[block, ip, minimum width=2.5cm, below=2.8cm of out_ip.south west, anchor=north west] (in_ip) {Inner IP Header\\(Privat 10.8.x.x)};
    \node[block, tcp, minimum width=2.5cm, right=0 of in_ip] (in_tcp) {Inner TCP/UDP\\(Port Aplikasi Asli)};
    \node[block, app, minimum width=3cm, right=0 of in_tcp] (in_tls) {App Metadata\\(\textit{TLS Magic Bytes})};
    \node[block, enc, minimum width=5.5cm, right=0 of in_tls] (in_app) {Data Aplikasi Aktual\\(\textit{Application Ciphertext})};

    % Judul Baris 2 (Diposisikan di tengah keseluruhan baris)
    \path (in_ip.north west) -- (in_app.north east) node[midway, above=0.4cm, font=\normalsize\bfseries] {Lalu Lintas di Dalam Terowongan (Antarmuka Virtual / \texttt{tun0})};

    % --- GARIS PROYEKSI (Menunjukkan Dekapsulasi) ---
    % pos=0.4 agar teks dekapsulasi naik sedikit dan tidak menimpa judul baris 2
    \draw[dashed, thick, gray, ->] (ovpn_payload.south west) -- (in_ip.north west) node[pos=0.4, left, align=center, font=\footnotesize\itshape] {Dekapsulasi\\oleh Daemon\\OpenVPN};
    \draw[dashed, thick, gray, ->] (ovpn_payload.south east) -- (in_app.north east);

    % --- KOTAK PEMBATAS tun0 ---
    % Membuat kotak putus-putus merah yang membungkus seluruh baris 2
    \begin{scope}[on background layer]
        \node[draw, dashed, red, ultra thick, inner sep=12pt, fill=red!5, fit=(in_ip) (in_app) (in_tls)] (tun0_box) {};
    \end{scope}

    % --- KURUNG KURAWAL (Area Ekstraksi Alpha) ---
    \draw[decorate,decoration={brace,amplitude=10pt,mirror},thick, blue]
        (in_ip.south west) -- (in_tls.south east) node[midway,below=15pt, font=\small\bfseries, blue] {Batas Ekstraksi Fitur Spasial 1D-CNN ($\alpha$)};

    % --- LABEL KOTAK PEMBATAS tun0 ---
    % Jarak diperjauh (below=1.2cm) agar tulisan merah di bawah berada di bawah teks kurung kurawal biru
    \node[red, below=1.2cm of tun0_box.south, font=\small\bfseries] {Batas Penangkapan Lalu Lintas Dataset ISCX 2016};

    \end{tikzpicture}
    }
    \caption{Visualisasi perbandingan anatomi enkapsulasi paket OpenVPN pada jaringan publik luar (\texttt{eth0}) dan titik batas penangkapan dataset pada antarmuka virtual (\texttt{tun0}). Diagram ini mengilustrasikan bagaimana representasi 1D-CNN ($\alpha$) berhasil mengekstrak pola spasial dari \textit{inner header} dan metadata jabat tangan aplikasi sebelum terenkripsi penuh.}
    \label{fig:openvpn_encapsulation_structure}
    \vspace{0.2cm}
    {\scriptsize\textit{*Sumber: Ilustrasi konseptual dengan bantuan AI}}
\end{figure}

Namun, perlu dipahami secara mendalam bahwa karena arsitektur ekstraksi fitur dalam penelitian ini beroperasi pada lingkungan data yang dipanen dari antarmuka virtual \texttt{tun0}, representasi biner mentah yang dievaluasi berada di dalam batas enkapsulasi OpenVPN tersebut. Seperti yang diilustrasikan pada Gambar \ref{fig:openvpn_encapsulation_structure}, paket-paket data yang diumpankan ke dalam model terhindar dari selubung \textit{outer header} UDP 1194. 

Kondisi ini menyisakan \textit{inner header} IPv4 (ditandai secara konstan oleh runtunan biner \texttt{0x4500}) beserta port internal asli aplikasi dalam bentuk yang transparan. Sementara itu, muatan aplikasi (\textit{payload}) di baliknya tetap berada dalam kondisi tersandi biner acak jika aplikasi dasar yang digunakan memanfaatkan enkripsi mandiri pada tingkat \textit{Transport-Level Security} (seperti runtunan \textit{magic bytes} \texttt{17 03 01} pada protokol HTTPS aplikasi \textit{Chat}). Karakteristik kombinasi antara stabilitas posisi spasial \textit{inner header} dan asimetri runtunan biner awal pada sisa \textit{payload} aplikasi inilah yang memvalidasi landasan ilmiah dari pembentukan komponen fitur hibrida.
%-----------------------------------------------------------------------------%
...
\begin{itemize}
    ...
    \item Waktu jeda antar-burst (inter-burst idle time): Statistik (Mean, Min, Max) dari waktu antar burst, sebagai indikator kelembaman protokol aplikasi tertentu \citep{Razooqi2025-bo, DraperGil2016}.
\end{itemize}
...
\subsection{Ekstraksi Representasi Fitur Level-Byte dari Raw Payload}

Pendekatan klasifikasi tradisional umumnya bergantung pada fitur statistik yang direkayasa secara manual (\textit{handcrafted features}) oleh pakar jaringan. Meskipun efektif, pendekatan ini rentan mengabaikan informasi struktural yang kaya di dalam paket data itu sendiri. Sebagai alternatif, pembelajaran representasi (\textit{representation learning}) memungkinkan model untuk secara otomatis mengekstrak fitur langsung dari data mentah atau \textit{raw payload} tanpa campur tangan manual \citep{sharma2024survey}.

Dalam analisis jaringan, aliran paket pada dasarnya dapat dipandang sebagai representasi sekuensial satu dimensi (\textit{1D byte sequence}). Keberhasilan analisis \textit{raw payload} pada lalu lintas terenkripsi didasarkan pada temuan bahwa urutan \textit{byte} ini masih menyimpan \textit{signature} implisit \citep{razooqi2025vpn}. Pola-pola laten ini sering kali terbentuk dari metadata awal sesi koneksi yang belum terenkripsi secara penuh (misalnya pada fase negosiasi protokol atau \textit{handshake}), ukuran pembingkaian (\textit{framing}), serta pola struktural tingkat rendah (\textit{low-level}) yang menjadi ciri khas perilaku aplikasi di balik terowongan VPN \citep{Lotfollahi2019}. 

Oleh karena itu, alih-alih mencoba mendekripsi paket untuk membaca teks di dalamnya (\textit{plaintext}), analisis \textit{raw payload} modern berfokus pada pengenalan sidik jari spasial dari susunan \textit{byte} tersebut. Dengan memproyeksikan urutan \textit{byte} mentah ini ke dalam ruang dimensi tinggi, model \textit{deep learning} mampu menangkap karakteristik mikroskopis dari lalu lintas jaringan. Representasi \textit{level-byte} inilah yang nantinya akan melengkapi fitur statistik makro (\textit{flow} dan \textit{burst}) untuk membangun profil lalu lintas yang komprehensif \citep{wang2017end}.
