...
\includegraphics[height=0.7\textheight, keepaspectratio]{assets/pics/macro_arch.PNG}
...
Dengan tiga tingkat klasifikasi ini, dataset ISCXVPN2016 menyediakan metode evaluasi yang komprehensif dan representatif untuk penelitian network traffic classification, terutama dalam konteks trafik terenkripsi yang semakin berkembang.

\subsection{Prosedur Pengumpulan Data dan Observasi Antarmuka}
\label{sec:bab3-prosedur-pengumpulan}

Untuk memenuhi validitas metodologi eksperimen, pemahaman mendalam mengenai prosedur pengumpulan data (\textit{data collecting}) dari dataset asli sangat krusial. Merujuk pada skema eksperimen yang dirancang oleh pembuat dataset \citep{draper2016characterization}, perekaman lalu lintas dilakukan secara langsung pada sisi \textit{host} klien menggunakan utilitas penangkap paket jaringan (\textit{packet capture}). Skenario pengumpulan data dibagi menjadi dua kondisi lingkungan utama:
\begin{enumerate}
    \item \textbf{Pengumpulan Trafik Non-VPN:} Aplikasi dieksekusi secara standar di atas sistem operasi \textit{host} klien. Paket data mengalir langsung dari kartu antarmuka jaringan fisik (\textit{Physical Network Interface Card}) menuju jaringan publik luar melalui perutean lokal standar kampus \citep{draper2016characterization}.
    \item \textbf{Pengumpulan Trafik VPN:} Pengguna mengaktifkan koneksi terowongan terenkripsi menggunakan protokol OpenVPN sebelum menjalankan aplikasi \citep{draper2016characterization}. OpenVPN bertindak sebagai penyedia jalur virtual aman yang membelokkan seluruh perutean lalu lintas biner dari aplikasi \citep{crist2015mastering}.
\end{enumerate}

Berdasarkan investigasi empiris yang dilakukan  terhadap file tangkapan paket mentah (\textit{raw pcap}) dalam dataset ISCX 2016, ditemukan sebuah karakteristik penting mengenai lokasi titik observasi penangkapan data. Lalu lintas kategori VPN dalam dataset ini tidak direkam pada kabel jaringan luar (sisi publik di luar komputer), melainkan ditangkap langsung pada tingkat antarmuka virtual internal sistem operasi, yaitu \textit{driver adapter} \texttt{tun0} OpenVPN. 

Fakta arsitektur perekaman ini dibuktikan melalui analisis silang (\textit{cross-examination}) terhadap dua sampel berkas \textit{instant messaging} dari repositori dataset, yaitu berkas \texttt{aim\_chat\_3a.pcap} (kondisi Non-VPN) dan berkas \texttt{vpn\_aim\_chat1a.pcap} (kondisi VPN). Hasil visualisasi pembedahan struktur paket menggunakan perangkat lunak Wireshark ditunjukkan pada Gambar \ref{fig:perbandingan_aim_wireshark}.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.95\textwidth]{assets/pics/wireshark_ip_compare.png}
    \caption{Perbandingan visibilitas alamat IP pada antarmuka penangkapan data. Bagian atas menunjukkan kondisi Non-VPN (antarmuka fisik dengan \textit{Destination IP} 131.202.240.87), sedangkan bagian bawah menunjukkan kondisi VPN yang direkam di dalam \texttt{tun0} (pergeseran \textit{Destination IP} menjadi 10.8.8.178).}
    \label{fig:perbandingan_aim_wireshark}
\end{figure}

Analisis struktural lebih lanjut menggunakan pustaka inspeksi (\texttt{scapy}) paket memperlihatkan anomali parameter teknis yang sangat kontras antara kedua kondisi perekaman tersebut. Perbandingan karakteristik struktural \textit{header} dan metadata antara berkas lalu lintas Non-VPN dan VPN dirangkum pada Tabel \ref{tab:komparasi_pcap_iscx}.

\begin{table}[h!]
\centering
\caption{Tabel Komparasi Karakteristik Parameter Berkas PCAP pada Dataset ISCX 2016}
\label{tab:komparasi_pcap_iscx}
\begin{tabular}{|l|p{5.5cm}|p{5.5cm}|}
\hline
\textbf{Atribut Jaringan} & \textbf{Trafik Standar (Non-VPN)} & \textbf{Trafik VPN (Virtual Interface \texttt{tun0})} \\ \hline
Nama Berkas Contoh & \texttt{aim\_chat\_3a.pcap} & \texttt{vpn\_aim\_chat1a.pcap} \\ \hline
Alamat IP Sumber & Alamat IP Publik Server Aplikasi asli (Contoh Server AIM: \texttt{205.188.12.91}) & Alamat IP Publik Server Aplikasi asli (Contoh Server AIM: \texttt{205.188.12.91})\\ \hline
Alamat IP Tujuan & Alamat IP Publik asli komputer \textit{host} (Blok Universitas: \texttt{131.202.240.87}) & Alamat IP Privat Virtual internal terowongan OpenVPN (\texttt{10.8.8.178}) \\ \hline
Port Transportasi & \textit{Port} asli milik layanan aplikasi (Contoh: TCP port 443 / port khusus) & \textit{Port} asli milik layanan aplikasi (Contoh: TCP port 443 / port khusus) \\ \hline
Visibilitas Konten & Terenkripsi pada tingkat lapisan aplikasi melalui protokol TLS/HTTPS & Terenkripsi pada tingkat lapisan aplikasi (TLS/HTTPS) dan mengalami pembatasan MTU internal VPN \\ \hline
\end{tabular}
\end{table}

Melalui representasi data pada Tabel \ref{tab:komparasi_pcap_iscx}, diperoleh sebuah \textit{insight} arsitektural: karena penangkapan data dilakukan dari dalam antarmuka virtual \texttt{tun0}, elemen \textit{header} asli secara administratif masih dapat terbaca oleh \textit{sniffer} lokal sebelum dibungkus sepenuhnya menjadi paket UDP port 1194 oleh subsistem enkapsulasi luar OpenVPN. 

Meskipun \textit{port} aplikasi masih terlihat pada antarmuka \texttt{tun0}, metode \textit{Deep Packet Inspection} (DPI) konvensional maupun aturan berbasis port tetap dipastikan gagal melakukan klasifikasi. Hal ini terjadi karena selain \textit{payload} yang tersandi acak oleh TLS, intervensi mesin VPN merusak karakteristik fisik lalu lintas data itu sendiri. Pembuktian visual mengenai distorsi fisik ini dapat diobservasi melalui fitur \textit{Packet Lengths} pada Gambar \ref{fig:perbandingan_stats_wireshark}.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.95\textwidth]{assets/pics/wireshark_stats_compare.png}
    \caption{Perbandingan statistik sebaran ukuran paket dan \textit{burst rate} antara trafik Non-VPN (atas) dan VPN (bawah). Terlihat jelas terjadinya distorsi ukuran dan ritme aliran data akibat intervensi mesin OpenVPN.}
    \label{fig:perbandingan_stats_wireshark}
\end{figure}

Merujuk pada Gambar \ref{fig:perbandingan_stats_wireshark}, meskipun aplikasi yang dieksekusi sama persis (\textit{AIM Chat}), ketika lalu lintas dialirkan melalui saluran VPN, terjadi pergeseran \textit{Behavioral Fingerprint} (sidik jari perilaku) yang ekstrem:
\begin{enumerate}
    \item \textbf{Distorsi Ukuran Paket:} Rata-rata ukuran paket (\textit{average packet length}) anjlok secara drastis (dari sekitar 600 \textit{bytes} menjadi sekitar 136 \textit{bytes}). Mayoritas paket terfragmentasi dan menumpuk pada rentang ukuran kecil akibat penyesuaian \textit{Maximum Transmission Unit} (MTU) internal OpenVPN.
    \item \textbf{Distorsi \textit{Burst Rate}:} Laju semburan data sesaat (\textit{burst rate}) juga mengalami perubahan ritme yang signifikan. Proses enkripsi kriptografis dan antrean (\textit{queueing buffer}) di latar belakang VPN telah mengubah metrik \textit{Inter-Arrival Time} (IAT) alami aplikasi.
\end{enumerate}

Pergeseran drastis pada ukuran dan ritme \textit{burst} inilah yang menjadi landasan fundamental mengapa pendekatan \textit{Machine Learning} mutlak diperlukan. Model \textit{Hybrid Flow Vector} tidak membaca teks identitas pada \textit{header}, melainkan bertugas mengekstrak pola sebaran statistik ini (Fitur Beta dan Gamma) untuk membedakan aplikasi—sebuah tugas kognisi pola (\textit{pattern recognition}) berdimensi tinggi yang tidak dapat dipecahkan oleh aturan heuristik biner statis.
...
Untuk mengotomatisasi pemrosesan pada seluruh dataset, sebuah skrip Python digunakan untuk memanggil SplitCap pada setiap file \texttt{.pcap}, untuk melakukan command \texttt{mono ./SplitCap.exe -r [pcap\_file\_path.pcap] -o [output\_dir] -s session -p 500}. Perlu dicatat penambahan argumen \texttt{-p 500} yang penting untuk membatasi sesi paralel dan menghindari galat ``Too many open files''.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{assets/pics/all_cleaning_process1.png}
    \caption{Ilustrasi alir pra-pemrosesan data, menunjukkan reduksi dari data mentah menjadi dataset final melalui Rule A, B, dan C.}
...
%-----------------------------------------------------------------------------%
\subsection{Skema Klasifikasi Bertingkat (\textit{Hierarchical Classification}) dan Pemilihan Sampel}
\label{subsec:skema_klasifikasi}
%-----------------------------------------------------------------------------%

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\textwidth]{assets/pics/distribusi_kategori.png} % Sesuaikan path folder gambar Anda
    \caption{Distribusi jumlah sampel untuk setiap kategori layanan VPN dan NonVPN pada dataset HFV pasca-integrasi.}
    \label{fig:distribusi_kategori}
\end{figure}

Berbeda dengan pendekatan yang memisahkan dataset secara fisik sejak awal ekstraksi, penelitian ini menggunakan pendekatan \textit{Single Source of Truth} berupa satu dataset gabungan utuh (\texttt{HFV\_dataset.csv}). Dataset ini memuat representasi fitur hibrida lengkap $\Omega = (\alpha \oplus \beta \oplus \gamma)$ yang berhasil diekstrak dari proses \textit{merging}. Pendekatan ini dipilih secara terencana untuk mencegah kebocoran data (\textit{data leakage}) dan memastikan konsistensi sampel pada setiap skenario pengujian.

Untuk mengevaluasi kinerja \textit{Hybrid Flow Vector} (HFV) pada berbagai tingkat granularitas, eksperimen dirancang menggunakan metode klasifikasi bertingkat (\textit{hierarchical classification}) yang disimulasikan melalui pemfilteran tingkat perangkat lunak (\textit{software-level subsetting}). Alur klasifikasi dieksekusi melalui tiga skenario pengujian utama:

\begin{enumerate}
    \item \textbf{Skenario 1: Klasifikasi Biner (Deteksi VPN)} \\
    Pada tahap awal, model dievaluasi untuk membedakan secara makro apakah suatu arus lalu lintas (\textit{flow}) dibungkus oleh terowongan VPN atau tidak. Tahap ini mengeksploitasi seluruh populasi dataset fitur HFV yang berhasil diekstrak, yaitu sebanyak 12.555 sampel (terdiri dari 9.804 sampel \textit{NonVPN} dan 2.751 sampel \textit{VPN}).

    \item \textbf{Skenario 2: Klasifikasi Kategori (Karakterisasi VPN)} \\
    Mensimulasikan cara kerja \textit{Intrusion Detection System} (IDS) di lingkungan produksi nyata, data yang diproses pada tahap ini direduksi hanya pada \textit{flow} yang telah diklasifikasikan sebagai lalu lintas VPN. Pemfilteran dilakukan untuk mengisolasi 2.751 sampel VPN tersebut. Model kemudian dilatih untuk mengidentifikasi enam kategori layanan (\textit{VoIP, File Transfer, Chat, Email, Streaming,} dan \textit{P2P}). Visualisasi sebaran kelas untuk skenario ini dapat dilihat pada Gambar \ref{fig:distribusi_kategori}.

\begin{table}[htbp]
    \centering
    \footnotesize
    \caption{Distribusi Sampel \textit{Flows} PCAP Awal (\textit{Pre-Merge})}
    \label{tab:distribusi_premerge}
    \captionsetup{font=small}
    ...
    \begin{tabular}{@{}llrrr@{}}
        \toprule
        \textbf{No} & \textbf{Aplikasi} & \textbf{VPN} & \textbf{NonVPN} & \textbf{Total} \\
        \midrule
        1 & Skype & 1.246 & 3.458 & 4.704 \\
        2 & Email & 137 & 1.764 & 1.901 \\
        3 & SCP & 0 & 1.741 & 1.741 \\
        4 & VOIPBuster & 53 & 1.143 & 1.196 \\
        5 & FTP & 101 & 749 & 850 \\
        6 & Facebook & 247 & 506 & 753 \\
        7 & Hangout & 325 & 393 & 718 \\
        8 & YouTube & 157 & 222 & 379 \\
        9 & BitTorrent & 363 & 0 & 363 \\
        10 & Vimeo & 44 & 224 & 268 \\
        11 & Spotify & 91 & 90 & 181 \\
        12 & Netflix & 63 & 102 & 165 \\
        13 & SFTP & 13 & 95 & 108 \\
        14 & Gmail & 0 & 70 & 70 \\
        15 & AIM Chat & 14 & 34 & 48 \\
        16 & ICQ & 20 & 22 & 42 \\
        \midrule
        \multicolumn{2}{r}{\textbf{Total Keseluruhan}} & \textbf{2.874} & \textbf{10.613} & \textbf{13.487} \\
        \bottomrule
    \end{tabular}%
    %}
\end{table}

    \item \textbf{Skenario 3: Klasifikasi Aplikasi Spesifik di Dalam VPN} \\
    Tugas dengan tingkat kesulitan tertinggi ini menargetkan identifikasi aplikasi spesifik (misalnya Skype vs Hangout) di dalam terowongan terenkripsi. Pada tahap ini, seleksi sampel (\textit{sample selection}) secara ketat diberlakukan berdasarkan analisis distribusi kelas untuk menghindari \textit{class imbalance} yang ekstrem.
\end{enumerate}

Sebelum proses ekstraksi dan integrasi fitur HFV dilakukan, raw data berupa PCAP \textit{flows} (\textit{pre-merge}) memiliki total 13.487 sampel. Rincian distribusi awal dari data \textit{pre-merge} ini disajikan pada Tabel \ref{tab:distribusi_premerge}.

Berdasarkan hasil analisis pasca-integrasi ($\alpha \oplus \beta \oplus \gamma$), terjadi penyisihan data alamiah pada \textit{flow} jaringan yang tidak memenuhi prasyarat kelengkapan tiga pilar fitur. Total sampel menyusut dari 13.487 \textit{raw} PCAP menjadi 12.555 baris observasi fitur yang utuh. Penjelasan mendetail mengenai mekanisme penggabungan fitur ini dan justifikasi teknis terkait faktor penyebab penyusutan data (seperti jumlah paket dari sebuah \textit{flow} yang terlalu sedikit untuk menghasilkan komponen \textit{alpha}) akan dibahas pada Sub-bab 3.3.5.2 mengenai Konsistensi Dataset dan Integritas Sampel.

Penyusutan dan distribusi natural dari dataset ISCX 2016 ini mengungkap adanya ketidakseimbangan kelas (\textit{class imbalance}) yang ekstrem pada sub-populasi VPN:
\begin{itemize}
    \item Terdapat aplikasi yang sama sekali tidak memiliki sampel VPN (\textit{zero-instance}) di dalam dataset, seperti \texttt{SCP} dan \texttt{Gmail}.
    \item Terdapat kelas minoritas dengan jumlah sampel VPN yang sangat tidak memadai untuk pelatihan model \textit{machine learning} yang \textit{robust} (misalnya \texttt{SFTP} dengan 11 sampel, \texttt{AIM Chat} dengan 14 sampel, \texttt{ICQ} dengan 20 sampel, dan \texttt{Vimeo} dengan 37 sampel).
\end{itemize}

Memaksakan inklusi kelas dengan sampel mikroskopis tersebut akan memicu fenomena \textit{overfitting} atau menyebabkan algoritma klasifikasi sepenuhnya mengabaikan kelas minoritas selama fase konvergensi. Oleh karena itu, penelitian ini memberlakukan ambang batas (\textit{threshold}) kelayakan secara empiris, di mana hanya aplikasi target dengan rekam data VPN lebih dari 100 sampel yang dipertahankan untuk Skenario 3. 

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{assets/pics/distribusi_aplikasi.png} % Sesuaikan path folder gambar Anda
    \caption{Distribusi sampel aplikasi spesifik. Garis putus-putus menunjukkan ambang batas minimum (\textgreater 100 sampel) VPN yang diperlukan untuk mencegah \textit{overfitting} pada pelatihan model klasifikasi.}
    \label{fig:distribusi_aplikasi}
\end{figure}

Sebagaimana diilustrasikan pada Gambar \ref{fig:distribusi_aplikasi}, pendekatan selektif ini pada akhirnya memfilter dan menyisakan aplikasi dominan seperti \texttt{Skype}, \texttt{BitTorrent}, \texttt{Hangout}, \texttt{Facebook}, dan \texttt{YouTube}, dan \texttt{Email} sebagai target klasifikasi \textit{fine-grained}. Keputusan pemfilteran ini krusial untuk memastikan bahwa evaluasi metrik kinerja kerangka kerja HFV tetap valid secara statistik dan tidak terdistorsi oleh anomali keterwakilan data.

\section{Perancangan dan Implementasi \textit{Hybrid Flow Vector} (HFV)}

\subsection{Pengantar metode HFV: $\Omega = (\alpha, \beta, \gamma)$}
...
Komponen $\alpha$ dirancang secara khusus untuk menangkap \textbf{pola mikroskopis spasial pada level byte} yang terkandung di dalam aliran data terenkripsi. Berbeda dengan analisis statistik konvensional ($\beta$ dan $\gamma$) yang mengekstrak fitur makroskopis temporal dari luar aliran, komponen $\alpha$ melakukan inspeksi langsung ke dalam susunan biner mentah lalu lintas jaringan tanpa melakukan proses dekripsi (\textit{decryption-free inspection}).

Meskipun lalu lintas data berada dalam kondisi tersandi, literatur keamanan siber membuktikan bahwa bagian awal dari aliran data (\textit{early-stage flows}) tidak sepenuhnya berupa biner acak tak berstruktur \citep{wang2017end, Lotfollahi2019}. Pada konteks dataset ISCX 2016, hasil pembedahan struktur \textit{Hex Dump} secara empiris membuktikan bahwa arsitektur \textit{1D-Convolutional Neural Networks} (1D-CNN) pada komponen $\alpha$ secara efektif memetakan ruang fitur laten dari dua elemen struktural krusial:
\begin{enumerate}
    \item \textbf{Struktur \textit{Inner Header} Jaringan:} Karena perekaman lalu lintas VPN dilakukan pada antarmuka virtual \texttt{tun0}, elemen \textit{header} jaringan asli milik aplikasi tetap dipertahankan pada awal blok memori. Observasi pada raw bytes menunjukkan setiap paket konsisten diawali dengan urutan biner statis seperti \texttt{0x4500} (penanda awal arsitektur IPv4). \textit{Offset} dan fluktuasi panjang \textit{header} IP/TCP ini menyediakan kerangka spasial yang stabil bagi algoritma jaringan saraf konvolusi.
    \item \textbf{Metadata Jabat Tangan Aplikasi (\textit{Protocol Magic Bytes}):} Sebelum enkripsi total menutupi seluruh blok komunikasi, pustaka aplikasi menyisipkan metadata protokol transparan. Sebagai contoh empiris pada aliran \textit{AIM Chat}, secara persisten ditemukan urutan \textit{magic bytes} \texttt{0x170301} (penanda muatan \textit{TLS 1.0 Application Data}) atau \texttt{0x170303} (TLS 1.2) yang menjembatani \textit{header} dan blok terenkripsi (\textit{ciphertext}). Karena setiap layanan daring dikembangkan dengan tumpukan pustaka (\textit{software stack}), mekanisme \textit{padding}, dan set \textit{Cipher Suite} yang berbeda, susunan biner awal pada fase negosiasi ini membentuk sidik jari struktural (\textit{fingerprint}) yang unik antar-kelas aplikasi.
\end{enumerate}

Bukti empiris mengenai keberadaan pola struktural biner laten ini dapat diobservasi langsung melalui inspeksi \textit{Hex Dump} Wireshark, seperti yang diilustrasikan pada Gambar \ref{fig:wireshark_alpha_bytes}.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.95\textwidth]{assets/pics/wireshark_alpha_bytes.png}
    \caption{Visualisasi jendela \textit{Hex Dump} dari paket \textit{AIM Chat} yang diekstraksi dari antarmuka \texttt{tun0}. Terlihat secara transparan struktur statis \textit{inner header} (awal \texttt{45 00}) yang berlanjut pada metadata protokol TLS (\texttt{17 03 XX}) sebelum berubah menjadi blok \textit{ciphertext} acak. Pola spasial tersembunyi inilah yang dipelajari oleh arsitektur 1D-CNN.}
    \label{fig:wireshark_alpha_bytes}
\end{figure}

\subsubsection{Representasi Input dan Normalisasi}
Agar pola struktural biner yang bervariasi ini dapat dikonsumsi oleh lapisan konvolusi \textit{Neural Network}, seluruh aliran paket harus ditransformasikan menjadi representasi matriks numerik dua dimensi yang simetris secara struktural. Prosedur penyiapan tensor input ini diimplementasikan melalui dua tahap berurutan:
\begin{enumerate}
    \item \textbf{Pemotongan Aliran (\textit{Flow Slicing}):} Model mengambil batas potong pada $N=10$ paket pertama yang membawa muatan data dari setiap iterasi sesi (\textit{flow}). Batasan empiris ini ditetapkan karena fase jabat tangan kriptografi (\textit{handshake}) dan inisialisasi sesi aplikasi seluruhnya terkonsentrasi pada segmen awal koneksi, sebelum berganti ke lalu lintas arus utama yang bersifat sepenuhnya statis atau acak.
    \item \textbf{Penyelarasan Dimensi (\textit{Padding and Truncation}):} Setiap paket dipotong secara sekuensial jika panjangnya melampaui, atau diisi dengan biner kosong (\textit{zero padding}) jika kurang, hingga memenuhi ukuran \textit{buffer} statis sebesar $L=784$ byte. Kapasitas 784 byte ditetapkan secara presisi berdasarkan analisis anatomis \textit{raw payload}. Observasi empiris membuktikan bahwa panjang \textit{header} jaringan bersifat fluktuatif (misalnya \textit{TCP header} yang bervariasi antara 20 hingga 32 byte akibat keberadaan \textit{TCP Options}). Fluktuasi ini menyebabkan letak \textit{magic bytes} bergeser secara dinamis (seperti pada \textit{offset} ke-40 untuk aplikasi \textit{Chat} dan \textit{offset} ke-52 untuk \textit{Email}). Dimensi $L=784$ memberikan ruang kanvas spasial yang memadai agar arsitektur 1D-CNN dapat memetakan pergeseran titik koordinat tersebut, menelan seluruh blok jabat tangan kriptografi yang umumnya berukuran 150-250 byte, sekaligus menjadikan sisa biner kosong (\textit{zero padding}) di akhir matriks sebagai bagian dari sidik jari keruangan (\textit{sparsity signature}) yang khas untuk setiap aplikasi.
\end{enumerate}

Secara visual, anatomi informasi struktural apa saja yang umumnya tertangkap di dalam rentang sampel observasi $L=784$ byte ini dapat dilihat pada Gambar \ref{fig:anatomy_784_bytes}. Jendela pengamatan sebesar ini menjamin model 1D-CNN tidak kehilangan konteks perpindahan dari data transparan ke \textit{ciphertext}.

\begin{figure}[h!]
    \centering
    \begin{tikzpicture}[font=\small, x=0.016cm, y=1.2cm]
        % --- Menggambar Kotak Utama (0 hingga 784) ---
        \draw[thick] (0,0) rectangle (784, 1);

        % --- Segmen 1: IPv4 Header (20 byte) ---
        \draw[fill=blue!20, draw=black] (0,0) rectangle (20, 1) node[pos=.5, rotate=90] {\tiny IPv4};

        % --- Segmen 2: TCP Header (20 byte) ---
        \draw[fill=green!20, draw=black] (20,0) rectangle (40, 1) node[pos=.5, rotate=90] {\tiny TCP};

        % --- Segmen 3: TLS Magic Bytes / Metadata (sekitar 80 byte) ---
        % Menggunakan \scriptsize dan text width agar tidak keluar kotak
        \draw[fill=yellow!30, draw=black] (40,0) rectangle (150, 1) node[pos=.5, align=center, font=\scriptsize, text width=1.6cm] {Metadata\\Jabat Tangan\\(TLS Records)};

        % --- Segmen 4: Ciphertext (Muatan Terenkripsi) ---
        \draw[fill=red!20, draw=black] (150,0) rectangle (650, 1) node[pos=.5, align=center] {Muatan Terenkripsi\\(\textit{Ciphertext Block})};

        % --- Segmen 5: Zero Padding ---
        % Menggunakan \scriptsize dan text width
        \draw[fill=gray!20, draw=black] (650,0) rectangle (784, 1) node[pos=.5, align=center, font=\scriptsize, text width=1.9cm] {Biner Kosong\\(\textit{Zero Padding})};

        % --- Kurung Kurawal Atas (Anotasi Kategori) ---
        % Disesuaikan menjadi multi-baris pada bagian yang sempit
        \draw[decorate,decoration={brace,amplitude=5pt},thick] (0,1.1) -- (150,1.1) node[midway,above=5pt, align=center, font=\scriptsize, text width=2.2cm] {Transparan\\(\textit{Header} \& \textit{Metadata})};
        
        \draw[decorate,decoration={brace,amplitude=5pt},thick] (150,1.1) -- (650,1.1) node[midway,above=5pt, align=center] {Buram / \textit{Opaque} (Terenkripsi)};
        
        \draw[decorate,decoration={brace,amplitude=5pt},thick] (650,1.1) -- (784,1.1) node[midway,above=5pt, align=center, font=\scriptsize, text width=2cm] {Penyelarasan\\(\textit{Padding})};

        % --- Garis Ukuran Bawah (0 - 784) ---
        \draw[|-|] (0,-0.3) -- (784,-0.3) node[midway,below] {Panjang Tensor Input Konvolusi (1D-CNN): $\mathbf{L = 784}$ \textbf{byte}};
        \node[below] at (0,-0.3) {0};
        \node[below] at (784,-0.3) {784};
    \end{tikzpicture}
    
    \caption{Ilustrasi konseptual pemetaan data paket ke dalam batas rentang \textit{buffer} tetap $L=784$ byte, yang mencakup komponen transparan (\textit{header}), penanda negosiasi aplikasi (\textit{TLS records}), hingga batasan blok \textit{ciphertext}.}
    \label{fig:anatomy_784_bytes}
\end{figure}

Langkah selanjutnya adalah tahapan normalisasi. Data nilai biner mentah (\textit{raw bytes}) berwujud array integer dasar dalam rentang dimensi $[0, 255]$. Rangkaian matriks ini kemudian dikompresi menjadi bilangan riil berskala $[0.0, 1.0]$. Reduksi skalar ini merupakan prosedur esensial dalam disiplin komputasi jaringan saraf tiruan untuk menghindarkan model dari masalah \textit{exploding gradients}, serta mereduksi kompleksitas waktu komputasi konvergensi. Jika $x_{i,j}$ merepresentasikan letak nilai integer desimal dari byte ke-$j$ pada paket ke-$i$, maka persamaannya dinyatakan seperti pada Persamaan \ref{eq:alpha_norm}.
...
Konklusi dari seluruh rangkaian proses prapemrosesan aliran ini adalah terbentuknya tensor dimensi tinggi $\mathbf{X} \in \mathbb{R}^{N \times L}$ yang siap diproses sebagai input bagi mekanisme penemuan fitur laten.
...
Secara formal, jika $P_{c2s}$ dan $P_{s2c}$ adalah himpunan paket pada arah keluar dan masuk, maka himpunan observasi didefinisikan pada Persamaan \ref{eq:beta_vector_collection}:

\begin{myequation}{Himpunan Observasi Komponen Statistik Fitur Beta}
\label{eq:beta_vector_collection}
\begin{aligned}
    \mathbf{Z}^{c2s} &= \left\{ \mathrm{Size}(p) \mid p \in P_{c2s} \right\} \\
    \mathbf{I}^{c2s} &= \left\{ t_{k+1} - t_k \mid p_k, p_{k+1} \in P_{c2s} \right\}
\end{aligned}
\end{myequation}
...
Dimana $\oplus$ adalah operasi konkatenasi. Dengan 11 statistik untuk 4 himpunan data ($11 \times 4 = 44$) ditambah 3 fitur global, total dimensi vektor $\mathbf{v}_{\beta}$ adalah \textbf{47 fitur}.
...
Analisis ini membagi sebuah \textit{flow} kontinu menjadi segmen-segmen diskrit yang disebut \textit{burst}, dipisahkan oleh periode diam (\textit{idle time}). Sama halnya dengan komponen $\beta$, analisis level burst ini menerapkan pendekatan bidireksional dengan memisahkan aliran paket menjadi arah Client-to-Server (C2S) dan Server-to Client (S2C).
...
Fungsi statistik $\mathcal{F}$ yang menghitung 9 nilai deskriptif (Count, Sum, Mean, Std, Min, Max, Median, P25, P75) diterapkan pada 4 variabel burst tersebut, menghasilkan 36 fitur. Karena perhitungan dilakukan secara bidireksional (C2S dan S2C), maka dihasilkan $36 \times 2 = 72$ fitur. Ditambah dengan 2 fitur yang mencatat total jumlah burst untuk masing-masing arah, komponen ini secara kumulatif berkontribusi \textbf{74 fitur} \citep{kotak2025vpn, Liu2024}.
...
\subsubsection{Definisi Vektor Hibrida (\textit{Hybrid Flow Vector})}
...
\begin{myequation}{Vektor \textit{Hybrid Flow Vector} (HFV)}
    \label{eq:hfv_concat}
...
Dimana $\mu$ adalah rata-rata dan $\sigma$ adalah standar deviasi dari data latih. Langkah ini krusial untuk mencegah bias model terhadap fitur dengan magnitudo besar \citep{scikitlearnStandardScaler}.
