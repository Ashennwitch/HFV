...
# The source code for the main cell, using a raw string to ensure indentation is preserved exactly.
...
# Split the string into a list of lines, as Jupyter stores source as a list of strings (usually with \n)
new_source_lines = [line + "\n" for line in new_source_code_str.splitlines()]
# Remove the last \n from the very last line if it exists to be clean, though Jupyter handles it.
if new_source_lines:
...
if len(big_cells) > 0:
    ...
    # Delete subsequent big cells in reverse order to avoid index shifts
    for idx in sorted(big_cells[1:], reverse=True):
        del nb["cells"][idx]

    with open(nb_path, "w") as f:
        json.dump(nb, f, indent=1)
    print("Notebook fixed and modified.")
else:
